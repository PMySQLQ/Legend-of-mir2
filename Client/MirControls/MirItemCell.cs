...
namespace Client.MirControls
{
    public sealed class MirItemCell : MirImageControl
    {
        ...
        public override void OnMouseClick(MouseEventArgs e)
        {
            ...
            switch (e.Button)
            {
                case MouseButtons.Right:
                    ...
                    if (GameScene.Scene.RefineDialog.Visible && GridType == MirGridType.Inventory && Item != null)
                    {
                        for (int i = 0; i < GameScene.Scene.RefineDialog.Grid.Length; i++)
                        {
                            ...
                            {
                                MoveItem();
                                GameScene.Scene.RefineDialog.Grid[i].MoveItem();
                                GameScene.Scene.RefineDialog.Grid[i].Redraw();
                                break;
                            }
                        }
                    }

                    if (GameScene.Scene.NPCDropDialog.Visible && GridType == MirGridType.Inventory)
                    {
                        MoveItem();
                        GameScene.Scene.NPCDropDialog.ItemCell.OnMouseClick(e);
                        GameScene.Scene.NPCDropDialog.ConfirmButton.OnMouseClick(e);
                    }


                    if (GameScene.Scene.StorageDialog.Visible && GridType == MirGridType.Inventory && Item != null)
                    {
                        for (int j = 0; j < GameScene.Scene.StorageDialog.Grid.Length; j++)
                        {
                            if (GameScene.Scene.StorageDialog.Grid[j].Item == null)
                            {
                                MoveItem();
                                GameScene.Scene.StorageDialog.Grid[j].MoveItem();
                                GameScene.Scene.StorageDialog.Grid[j].Redraw();
                                break;
                            }
                        }
                    }
                    if (GameScene.Scene.StorageDialog.Visible && this.GridType == MirGridType.Storage && this.Item != null)
                    {
                        for (int k = 0; k < GameScene.Scene.InventoryDialog.Grid.Length; k++)
                        {
                 
                            if (GameScene.Scene.InventoryDialog.Grid[k].Item == null)
                            {
                                MoveItem();
                                GameScene.Scene.InventoryDialog.Grid[k].MoveItem();
                                GameScene.Scene.InventoryDialog.Grid[k].Redraw();
                                break;
                            }
                        }
                    }     
                    if (GameScene.Scene.TrustMerchantDialog.ItemCell.Visible && GridType == MirGridType.Inventory && Item != null)
                    {
                        MoveItem();
                        GameScene.Scene.TrustMerchantDialog.ItemCell.MoveItem();
                        GameScene.Scene.TrustMerchantDialog.ItemCell.Redraw();
                    }
                    UseItem();
                    break;
        ...
        public void UseItem()
        {
            ...
            switch (Item.Info.Type)
            {
                ...
                case ItemType.项链:
                    ...
                    break;
                case ItemType.手镯:
                    if ((dialog.Grid[(int)EquipmentSlot.BraceletR].Item == null || dialog.Grid[(int)EquipmentSlot.BraceletR].Item.Info.Type == ItemType.护身符) && dialog.Grid[(int)EquipmentSlot.BraceletR].CanWearItem(actor, Item))
                    {
                    ...
                    if (GameScene.SelectedItem != null && !GameScene.SelectedItem.Info.IsFishingRod && GameScene.SelectedItem.Info.Type != ItemType.坐骑)
                    {
                        switch (Item.Info.Shape)
                        {
                            case 1:
                                ...
                                break;
                            case 2:
                                if (GameScene.SelectedItem.Info.Type != ItemType.护甲) return;
                                break;
                            case 3:
        ...
        public void RemoveHeroItem()
        {
            ...
            if (Item == null || count < 1 || (MapObject.Hero.RidingMount && Item.Info.Type != ItemType.火把)) return;
            ...
            if (GameScene.SelectedCell != null)
            {
                ...
                switch (GridType)
                {
                    ...
                    case MirGridType.HeroMPItem:
                        ...
                        if (GameScene.Hero == null || GameScene.Hero.Dead)
                            return;
                        if (GameScene.SelectedCell.Item.Info.Type != ItemType.药剂 || GameScene.SelectedCell.Item.Info.Shape > 1)
                            return;
        ...
        private void UpdateGradeImage()
        {
            if (Item != null)
                switch (Item.Info.Grade)
                {
                    ...
                    case ItemGrade.史诗:
                        GradeEffect.Animated = true;
                        GradeEffect.Index = 70;
                        GradeEffect.Visible = true;
                        break;
                    case ItemGrade.传奇:
                        GradeEffect.Animated = true;
                        GradeEffect.Index = 50;
                        GradeEffect.Visible = true;
                        break;
                    case ItemGrade.英雄:
                        GradeEffect.Animated = true;
