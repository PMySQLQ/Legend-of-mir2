...
namespace Client.MirScenes
{
    public sealed class GameScene : MirScene
    {
        ...
        private void UpdateMouseCursor()
        {
            ...
            {
                if (GameScene.SelectedCell != null && GameScene.SelectedCell.Item != null && GameScene.SelectedCell.Item.Info.Type == ItemType.宝玉 && CMain.Ctrl)
                {
                ...
                for (int i = MapControl.Objects.Count - 1; i >= 0; i--)
                {
                    ...
                    switch (p.Type)
                    {
                        ...
                        case DamageType.MagicalHit://神奇一击
                            ...
                            break;
                        case DamageType.GreenPoison://绿色毒药
                        ...
                        case DamageType.Block:
                            obj.Damages.Add(new Damage("Block", 1200, obj.Race == ObjectType.Player ? Color.Yellow : Color.White, 100));
                            break;
                        case DamageType.Resist://抵抗
                            obj.Damages.Add(new Damage("抵抗", 1200, obj.Race == ObjectType.Player ? Color.Yellow : Color.White, 100));
                            break;
                    }
                }
            }
        ...
        private void ObjectProjectile(S.ObjectProjectile p)
        {
            ...
            switch (p.Spell)
            {
                case Spell.FireBounce:
                    {
                        SoundManager.PlaySound(20000 + (ushort)Spell.大火球 * 10 + 1);
        ...
        {            
            ...
            NewHeroDialog.ArcherButton.Visible = p.CanCreateClass[(int)MirClass.弓手];
        ...
        public void AddItem(UserItem item)
        {
            ...
            if (item.Info.Type == ItemType.药剂 || item.Info.Type == ItemType.卷轴 || (item.Info.Type == ItemType.脚本 && item.Info.Effect == 1))
            {
        ...
        public MirControl NameInfoLabel(UserItem item, bool inspect = false, bool hideDura = false)
        {
            ...
            MirLabel nameLabel = new MirLabel
            {
                ...
                OutLine = true,
                Parent = ItemLabel,
                Text = HoverItem.Info.Grade != ItemGrade.无 ? string.Format("{0}{1}{2}", HoverItem.Info.FriendlyName, "\n", GradeString) : HoverItem.Info.FriendlyName,
            };
        ...
        public MirControl AttackInfoLabel(UserItem item, bool Inspect = false, bool hideAdded = false)
        {
            ...
            if (minValue != 0 || addValue != 0)
            {
                ...
                else if (realItem.Type == ItemType.药剂 && realItem.Shape == 5)
                {
        ...
        public MirControl DefenceInfoLabel(UserItem item, bool Inspect = false, bool hideAdded = false)
        {
            ...
            if (HoverItem.Info.Type != ItemType.MonsterSpawn)
            {
                ...
                if (minValue > 0 || maxValue > 0 || addValue > 0)
                {
                    ...
                    MirLabel MAXHPLabel = new MirLabel
                    {
                        ...
                        OutLine = true,
                        Parent = ItemLabel,
                        //Text = string.Format(realItem.Type == ItemType.药剂 ? "HP + {0} Recovery" : "MAXHP + {0}", minValue + addValue)
            ...
            if (minValue > 0 || maxValue > 0 || addValue > 0)
            {
                ...
                MirLabel MAXMPLabel = new MirLabel
                {
                    ...
                    OutLine = true,
                    Parent = ItemLabel,
                    //Text = string.Format(realItem.Type == ItemType.药剂 ? "MP + {0} Recovery" : "MAXMP + {0}", minValue + addValue)
        ...
        public MirControl WeightInfoLabel(UserItem item, bool Inspect = false)
        {
            ...
            if (HoverItem.Info.Type == ItemType.变身 && HoverItem.Info.Durability > 0)
            {
                count++;
                MirLabel TNRLabel = new MirLabel
                {
        ...
        public MirControl AwakeInfoLabel(UserItem item, bool Inspect = false)
        {
            ...
            if (HoverItem.Awake.GetAwakeValue() > 0)
            {
                ...
                MirLabel AWAKE_TOTAL_VALUELabel = new MirLabel
                {
                    ...
                    OutLine = true,
                    Parent = ItemLabel,
                    Text = string.Format(realItem.Type != ItemType.护甲 ? "{0} + {1}~{2}" : "MAX {0} + {1}", HoverItem.Awake.Type.ToString(), HoverItem.Awake.GetAwakeValue(), HoverItem.Awake.GetAwakeValue())
                };
            ...
            if (HoverItem.Awake.GetAwakeLevel() > 0)
            {
                ...
                for (int i = 0; i < HoverItem.Awake.GetAwakeLevel(); i++)
                {
                    MirLabel AWAKE_LEVEL_VALUELabel = new MirLabel
                    {
                        ...
                        OutLine = true,
                        Parent = ItemLabel,
                        Text = string.Format(realItem.Type != ItemType.护甲 ? "Level {0} : {1} + {2}~{3}" : "Level {0} : MAX {1} + {2}~{3}", i + 1, HoverItem.Awake.Type.ToString(), HoverItem.Awake.GetAwakeLevelValue(i), HoverItem.Awake.GetAwakeLevelValue(i))
                    };
        ...
        public MirControl BindInfoLabel(UserItem item, bool Inspect = false, bool hideAdded = false)
        {
            ...
            //if ((HoverItem.Info.CanAwakening != true) && (HoverItem.Info.Type != ItemType.宝玉))
            //{
            //    count++;
    ...
    public sealed class MapControl : MirControl
    {
        ...
        public static void CancelCanRun()
        {
            if (!MapButtons.HasFlag(MouseButtons.Right))
                GameScene.CanRun = false;
        }
        ...
        private void CheckInput()
        {
            ...
            if (MapObject.TargetObject != null && !MapObject.TargetObject.Dead)
            {
                    ...
                    (!MapObject.TargetObject.Name.EndsWith(")") && MapObject.TargetObject is MonsterObject))
                {
                    ...
                    if ((User.Class == MirClass.弓手 || User.Class == MirClass.暗鬼弓手) && User.HasClassWeapon && !User.RidingMount && !User.Fishing)//ArcherTest - non aggressive targets (player / pets)
                    {
                    ...
                    case MouseButtons.Left:
                        ...
                        if (CMain.Shift)
                        {
                            if (CMain.Time > GameScene.AttackTime && CanRideAttack()) //ArcherTest - shift click
                            {
                                ...
                                if ((User.Class == MirClass.弓手 || User.Class == MirClass.暗鬼弓手) && User.HasClassWeapon && !User.RidingMount && !User.Poison.HasFlag(PoisonType.Dazed))
                                {
                                    if (target != null)
                                    {
                        ...
                        if (MapObject.MouseObject is MonsterObject && (User.Class == MirClass.弓手 || User.Class == MirClass.暗鬼弓手) && MapObject.TargetObject != null && !MapObject.TargetObject.Dead && User.HasClassWeapon && !User.RidingMount) //ArcherTest - range attack
                        {
                            if (Functions.InRange(MapObject.MouseObject.CurrentLocation, User.CurrentLocation, Globals.MaxAttackRange))
                            {
                                if (CMain.Time > GameScene.AttackTime)
            ...
            if ((User.Class == MirClass.弓手 || User.Class == MirClass.暗鬼弓手) && User.HasClassWeapon && (MapObject.TargetObject is MonsterObject || MapObject.TargetObject is PlayerObject)) return; //ArcherTest - stop walking
        ...
        public void UseMagic(ClientMagic magic, UserObject actor)
        {
            ...
            switch (magic.Spell)
            {
                ...
                case Spell.捕缚术:
                    ...
                    break;
                case Spell.拔刀术:
                    if (actor.GetMagic(Spell.拔刀术).Level <= 1 && actor.IsDashAttack() == false)
                    {
                        actor.ClearMagic();
                        return;
                    }
