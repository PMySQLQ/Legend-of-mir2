    ...
    public sealed class GameShopDialog : MirImageControl
    {
        ...
        public void GetCategories()
        {
            ...
            for (int i = 0; i < shopList.Count; i++)
            {
                ...
                {
                    if (shopList[i].Class == ClassFilter || shopList[i].Class == "所有" || ClassFilter == "所有")
                    {
                        if (SectionFilter == "所有" || SectionFilter == "热销" && shopList[i].TopItem || SectionFilter == "打折" && shopList[i].Deal || SectionFilter == "新品" && shopList[i].Date > CMain.Now.AddDays(-7))
