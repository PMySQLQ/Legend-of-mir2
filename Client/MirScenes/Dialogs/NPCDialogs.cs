...
namespace Client.MirScenes.Dialogs
{
    ...
    public sealed class NPCAwakeDialog : MirImageControl
    {
        ...
        public void Awakening()
        {
            if (CheckNeedMaterials())
            {
                ...
                if (type != AwakeType.None)
                {
                    Network.Enqueue(new C.Awakening { UniqueID = Items[0].UniqueID, Type = type,
                        ExtendMaterials = new ulong[]
                        {
                            (NPCAwakeDialog.Items[5] == null) ? 0 : NPCAwakeDialog.Items[5].UniqueID,
                            (NPCAwakeDialog.Items[6] == null) ? 0 : NPCAwakeDialog.Items[6].UniqueID
                        }
                    });
            ...
            {
                MirMessageBox messagea = new MirMessageBox("请在材料框内放入不少于所需数量的材料", MirMessageBoxButtons.YesNo);
                messagea.Show();
                return;
