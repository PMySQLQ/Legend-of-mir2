...
namespace ClientPackets
{
    ...
    public sealed class Awakening : Packet
    {
        ...
        protected override void ReadPacket(BinaryReader reader)
        {
            ...
            int count = reader.ReadInt32();
            ExtendMaterials = new ulong[count];
            for (int i = 0; i < count; i++)
            {
                ExtendMaterials[i] = reader.ReadUInt64();
            }
        }
        protected override void WritePacket(BinaryWriter writer)
        {
            ...
            writer.Write(ExtendMaterials.Length);
            for (int i = 0; i < ExtendMaterials.Length; i++)
            {
                writer.Write(ExtendMaterials[i]);
            }
        }
    }
