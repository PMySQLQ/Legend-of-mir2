...
namespace Server.MirObjects
{
    public class HumanObject : MapObject
    {
        ...
        public override void Process()
        {
            ...
            //if (CellTime + 700 < Envir.Time) _stepCounter = 0;//Starting problem
            ...
            {
                FlamingSword = false;
                Enqueue(new S.SpellToggle { ObjectID = ObjectID, Spell = Spell.烈火剑法, CanUse = false });
            }
        ...
        protected virtual void ProcessBuffs()
        {
            ...
            for (int i = Buffs.Count - 1; i >= 0; i--)
            {
                ...
                switch (buff.Type)
                {
                    ...
                    case BuffType.HumUp:
                        Humup = true;
                        if (!Humup) buff.FlagForRemoval = true;
                        break;
                    case BuffType.Lover:
                        lover = true;
            ...
            if (Class > MirClass.弓手 && !Humup)
            {
                ...
                switch (Class)
                {
                    case MirClass.碧血战士:
                        stats[Stat.HP] = +220;
                        stats[Stat.MP] = +130;
                        stats[Stat.HealthRecovery] = +10;
                        stats[Stat.SpellRecovery] = +10;
                        stats[Stat.BagWeight] = +80;
                        break;
                    case MirClass.虹玄法师:
                        stats[Stat.HP] = +140;
                        stats[Stat.MP] = +210;
                ...
                AddBuff(BuffType.HumUp, this, 0, stats, true);
            }
        ...
        public void RefreshStats()
        {
            ...
            AttackSpeed = 1400 - ((Stats[Stat.AttackSpeed] * 30) + Math.Min(370, (Level * 14)));
        ...
        private void RefreshItemSetStats()
        {
            foreach (var s in ItemSets)
            {
                if ((s.Set == ItemSet.Smash) &&
                    ((s.Type.Contains(ItemType.戒指) && s.Type.Contains(ItemType.手镯)) || (s.Type.Contains(ItemType.戒指) && s.Type.Contains(ItemType.项链)) || (s.Type.Contains(ItemType.手镯) && s.Type.Contains(ItemType.项链))))
                {
                ...
                if ((s.Set == ItemSet.DarkGhost) && (s.Type.Contains(ItemType.项链)) && (s.Type.Contains(ItemType.手镯)))
                {
                    Stats[Stat.HP] += 25;
                }
        ...
        public void Attack(MirDirection dir, Spell spell)
        {
            ...
            if (!Slaying)
            {
                ...
                {
                    Slaying = true;
                    Enqueue(new S.SpellToggle { ObjectID = ObjectID, Spell = Spell.攻杀剑术, CanUse = Slaying });
                }
            }
        ...
        public void Magic(Spell spell, MirDirection dir, uint targetID, Point location)
        {
            ...
            if (spell == Spell.野蛮冲撞 && Race == ObjectType.Hero)
                dir = Direction;
            ...
            switch (target.Race)
            {
                ...
                case ObjectType.Hero:
                    ...
                    if (target.IsFriendlyTarget(this))
                    {
                        ...
                        else if (((HumanObject)target).Class == MirClass.法师 || ((HumanObject)target).Class == MirClass.弓手)
                        {
        ...
        protected void CompleteAttack(IList<object> data)
        {
            ...
            if (FatalSword)
            {
                ...
                FatalSword = false;
                var magic = GetMagic(Spell.绝命剑法);
                if (magic != null) LevelMagic(magic);
            }
            if (userMagic != null && finalHit)
            {
                if (userMagic.Spell == Spell.双龙斩)
                {
        ...
        public void SpellToggle(Spell spell, SpellToggleState state)
        {
            ...
            switch (spell)
            {
                ...
                case Spell.烈火剑法:
                    ...
                    Enqueue(new S.SpellToggle { ObjectID = ObjectID, Spell = Spell.烈火剑法, CanUse = true });
                    ChangeMP(-cost);
                    break;
                case Spell.天务:
