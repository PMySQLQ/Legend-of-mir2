...
namespace Server.MirObjects
{
    public class PlayerObject : HumanObject
    {
        ...
        private void StartGameSuccess()
        {
            ...
            if (Info.Thrusting) Enqueue(new S.SpellToggle { ObjectID = ObjectID, Spell = Spell.刺杀剑术, CanUse = true });
            if (Info.HalfMoon) Enqueue(new S.SpellToggle { ObjectID = ObjectID, Spell = Spell.半月弯刀, CanUse = true });
                ...
                for (int x = CurrentLocation.X - Globals.DataRange; x <= CurrentLocation.X + Globals.DataRange; x++)
                {
                    ...
                    for (int i = 0; i < cell.Objects.Count; i++)
                    {
                        ...
                        else if (ob.Race == ObjectType.Spell)
                        {
                            ...
                            if ((obSpell.Spell != Spell.爆阱) || (obSpell.Caster != null && IsFriendlyTarget(obSpell.Caster)))
            ...
            else if (message.StartsWith("@"))
            {
                ...
                switch (parts[0].ToUpper())
                {
                    case "Deleteposition"://Fixed-point
                        ...
                        //ReceiveChat(string.Format("{0}",Index), ChatType.System);
        ...
        {
            // _stepCounter = 0;//Starting problem
        ...
        public void EquipSlotItem(MirGridType grid, ulong id, int to, MirGridType gridTo, ulong idTo)
        {
            ...
            if ((item.Info.IsFishingRod || item.Info.Type == ItemType.坐骑) && temp.Info.Type == ItemType.插座)
            {
                Enqueue(p);
                return;
            }
                    ...
                    switch (item.Info.Shape)
                    {
                        ...
                        case 202://位置记忆书
                            try
                            {
                                ...
                                {
                                    ReceiveChat(string.Format("允许最多定位{0}个地图坐标", Globals.MaxPositionMove), ChatType.Hint);
                                    Enqueue(p);
                                    return;
                                }
                                if (CurrentMap.Info.NoPositionMove)
                                {
                                    //Enqueue(new S.ErrorReopt { result = 0 });
                                    ReceiveChat(string.Format("无法定位当前地图坐标"), ChatType.Hint);
                                    Enqueue(p);
                                    return;
                                }
                            ...
                            catch (Exception ex)
                            {
                                ...
                                MessageQueue.Enqueue(Info.Name);
                            }
                            break;
                    }
                    break;
        ...
        private bool Match(AuctionInfo info)
        {
            return (UserMatch || !info.Expired && !info.Sold)
                ...
                && ((MatchType == ItemType.无 || info.Item.Info.Type == MatchType)
        ...
        public void GetMarket(string name, ItemType type)
        {
            ...
            if (MarketPanelType == MarketPanelType.GameShop)
            {
                //Search = Envir.GameShopList.Where(x => (MatchType == ItemType.无 || x.Info.Type == MatchType)
        ...
        public void Awakening(ulong UniqueID, AwakeType type, ulong[] ExtendMaterials)
        {
            ...
            for (int i = 0; i < Info.Inventory.Length; i++)
            {
                ...
                if (HasAwakeningNeedMaterials(item, type))
                {
                    ...
                    for (int idx = 0; idx < ExtendMaterials.Length; idx++)
                    {
                        for (int j = 0; j < Info.Inventory.Length; j++)
                        {
                            if (Info.Inventory[j] != null)
                            {
                                //提高觉醒几率
                                if (Info.Inventory[j].UniqueID == ExtendMaterials[idx] && Info.Inventory[j].Info.Shape == 200 && Info.Inventory[j].Info.Type == ItemType.觉醒)
                                {
                                    a = (byte)Info.Inventory[j].Info.Effect;
                                    Enqueue(new S.DeleteItem { UniqueID = Info.Inventory[j].UniqueID, Count = Info.Inventory[j].Count });
                                    Info.Inventory[j] = null;
                                }

                                //提高点数几率
                                else if (Info.Inventory[j].UniqueID == ExtendMaterials[idx] && Info.Inventory[j].Info.Shape == 201 && Info.Inventory[j].Info.Type == ItemType.觉醒)
                                {
                                    b = (byte)Info.Inventory[j].Info.Effect;
        ...
        public void DisassembleItem(ulong UniqueID)
        {
            ...
            for (int i = 0; i < Info.Inventory.Length; i++)
            {
                ...
                if (Info.AccountInfo.Gold >= item.DisassemblePrice())
                {
                    ...
                    {
                        if (drop.Item.Grade == item.Info.Grade - 1 || drop.Item.Grade == item.Info.Grade + 1)
                        {
        ...
        public bool HasAwakeningNeedMaterials(UserItem item, AwakeType type)
        {
            ...
            byte[] needCount = new byte[2] { materialCount[0], materialCount[1] };

            for (int i = 0; i < Info.Inventory.Length; i++)
            {
                UserItem materialItem = Info.Inventory[i];
                //Calculate the quantity of materials in the package
                if (materialItem != null)
                {
                    if (materialItem.Info.Grade == item.Info.Grade && materialItem.Info.Type == ItemType.觉醒)
                    {
                        if (materialItem.Info.Shape == ((int)type - 1) && materialCount[0] - currentCount[0] != 0)
                        {
                            //if (materialItem.Count <= materialCount[0] - currentCount[0])
                            if (materialItem.Count <= needCount[0])
                            {
                                currentCount[0] += (byte)materialItem.Count;
                                needCount[0] -= currentCount[0];
                            }
                            //else if (materialItem.Count > materialCount[0] - currentCount[0])
                            else if (materialItem.Count > needCount[0])
                            {
                                currentCount[0] += needCount[0];
                                needCount[0] = 0;
                            }
                        }
                        else if (materialItem.Info.Shape == 100 && materialCount[1] - currentCount[1] != 0)
                        {
                            //if (materialItem.Count <= materialCount[1] - currentCount[1])
                            if (materialItem.Count <= needCount[1])
                            {
                            ...
                            else if (materialItem.Count > needCount[1])
                            {
                                currentCount[1] += needCount[1];
                                needCount[1] = 0;
                            }
                        }
                    }
            ...
            for (int i = 0; i < Info.Inventory.Length; i++)
            {
                ...
                {
                    if (Info.Inventory[i].Info.Grade == item.Info.Grade && Info.Inventory[i].Info.Type == ItemType.觉醒)
                    {
                        if (Info.Inventory[i].Info.Shape == ((int)type - 1) &&  currentCount[0] > 0)
                        {
                        ...
                        else if (Info.Inventory[i].Info.Shape == 100 && currentCount[1] > 0)
                        {
        ...
        public void MemoryLocation(string name, int color)
        {
            try
            {
                ...
                if (CurrentMap.Info.NoPositionMove)
                {
                    Enqueue(new S.Awakening { result = 10 });
                    return;
                }
                ...
                {
                    UserItem item = Info.Inventory[i];
                    if (item == null || item.Info.Type != ItemType.卷轴) continue;
        ...
        public void PositionMove(int Index)
        {
            try
            {
                ...
                if (Info.MyTeleportInfo.Count > 0 && Index <= Info.MyTeleportInfo.Count)
                {
                    ...
                    if (tempmap.Info.NoPositionMove)
                    {
                        Enqueue(new S.Awakening { result = 10 });
                        return;
                    }

                    Info.AccountInfo.Gold -= 3000;
                    Enqueue(new S.LoseGold { Gold = 3000 });
