...
namespace Server
{
    public partial class MonsterInfoForm : Form
    {
        ...
        {
            string str = this.MonsterNameTextBox.Text.Trim();
            Regex regex = new Regex("^\\d+$");
            foreach (MirConnection mirConnection in SMain.Envir.Connections)
            {
                if (mirConnection.Player.IsGM)
                {
                    if (this.textBoxAmount.Text.Trim() != string.Empty)
                    {
                        if (regex.IsMatch(this.textBoxAmount.Text.Trim()))
                        {
                            if (int.Parse(this.textBoxAmount.Text.Trim()) >= 1)
                            {
                                if (mirConnection.Player.CurrentMap.ValidPoint(mirConnection.Player.Front))
                                {
                                    mirConnection.Player.Chat("@mob  " + str + " " + this.textBoxAmount.Text.Trim());
                                }
                                else
                                {
                                    mirConnection.Player.ReceiveChat("刷怪位置不是当前地图的有效点.", ChatType.System);
                                }
                    ...
                    else
                    {
                        if (mirConnection.Player.CurrentMap.ValidPoint(mirConnection.Player.Front))
                        {
                            mirConnection.Player.Chat("@mob  " + str);
                        }
                        else
                        {
                            mirConnection.Player.ReceiveChat("刷怪位置不是当前地图的有效点.", ChatType.System);
        ...
        private void btKillMob_Click(object sender, EventArgs e)
        {
            foreach (MirConnection mirConnection in SMain.Envir.Connections)
            {
                if (mirConnection.Player.IsGM)
                {
                    mirConnection.Player.Chat("@CLEARMOB");
                }
            }
        }

        private void buttonKill14Mob_Click(object sender, EventArgs e)
        {
            foreach (MirConnection mirConnection in SMain.Envir.Connections)
            {
                if (mirConnection.Player.IsGM)
                {
                    mirConnection.Player.Chat("@ONLYKILLMONSTER");
                }
            }
        }
        ...
        private void KillPlayerButton_Click(object sender, EventArgs e)
        {
            foreach (MirConnection mirConnection in SMain.Envir.Connections)
            {
                if (mirConnection.Player.IsGM)
                {
                    mirConnection.Player.Chat("@ONLYKILLPLAYER");
                }
            }
        }

        private void KillMemberButton_Click(object sender, EventArgs e)
        {
            foreach (MirConnection mirConnection in SMain.Envir.Connections)
            {
                if (mirConnection.Player.IsGM)
                {
                    mirConnection.Player.Chat("@ONLYKILLMEMBER");
                }
            }
        }
    }
